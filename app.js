    ...
    return new Promise((resolve, reject) => {
      ...
      https
        .get(assembleUrl, (res) => {
          ...
          //chunk라고 작성되어있는 이유는 데이터를 조각조각 받기 때문으로
          //데이터를 조각조각받아서 body에 저장한다
          res.on("data", (chunk) => {
            body += chunk;
            //body = body+chunk와 같다.
          });
          ...
          res.on("end", () => {
            ...
            //fullfilled(이행)할 때 까지 차곡차곡 쌓은 body를 resolve()함수에 넘겨준다.
            resolve(data);
          });
          //아래는 단순히 이벤트 처리
        })
...
const main = async () => {
  ...
  try {
    ...
    //main()함수는 본래 비동함수가 아니지만,
    //async 키워드를 사용하여 비동기함수로 만들었으므로
    //await 키워드를 사용하여 비동기 함수를 동기적으로 사용할 수 있다.

    const types = await pokemonData.types.map((typeInfo) => typeInfo.type.name);
    //.map() 메서드로 피카츄의 몬스터 타입을 가져오는 것도 await을 작성해
    //순차적으로 실행되는 가독성을 확보했다.
